...
export default function GlossaryAiRecommendationModal({
  ...
  const handleAiRecommendation = async () => {
    ...
    try {
      ...
      if (result.success && result.data?.recommendations) {
        setAiRecommendations(
          result.data.recommendations.map((rec: any) => ({ ...rec, selected: false })),
        )
      } else {
  ...
  const toggleRecommendationSelection = (index: number) => {
    setAiRecommendations((prev) => prev.map((r, i) => (i === index ? { ...r, selected: !r.selected } : r)))
  }
  ...
  const addSelectedRecommendations = async () => {
    ...
    try {
      ...
      const addedGlossaries: (Tables<'glossaries'> & { glossary_links: any[] })[] = []
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-[80vh] overflow-y-auto">
        ...
        {aiRecommendations.length === 0 && !aiError && (
          <>
            {!aiLoading && <p className="text-muted-foreground mb-6">{t('glossary.ai_modal_desc')}</p>}
        ...
        {aiRecommendations.length > 0 && !aiError && (
          <>
            <p className="text-muted-foreground mb-4">
              ...
              {aiRecommendations.some((r) => r.selected) && ` (${aiRecommendations.filter((r) => r.selected).length}개 선택됨)`}
            </p>
              ...
              {aiRecommendations.map((rec, index) => (
                <div
                  ...
                  className={`border rounded-lg p-4 cursor-pointer transition-colors ${
                    rec.selected ? 'border-primary bg-primary/5' : 'border-gray-200 hover:border-gray-300'
                  }`}
                  ...
                  <div className="flex items-start gap-3">
                    <div
                      className={`mt-1 w-5 h-5 rounded border-2 flex items-center justify-center cursor-pointer transition-colors ${
                        rec.selected ? 'bg-primary border-primary text-white' : 'border-gray-300 hover:border-gray-400'
                      }`}
                      onClick={(e) => {
                        e.stopPropagation()
                    ...
                    <div className="flex-1" onClick={() => toggleRecommendationSelection(index)}>
                      ...
                      <p className="text-sm text-muted-foreground mb-2">{rec.definition}</p>
                      {rec.examples && (
            ...
            <div className="flex justify-between">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={handleAiRecommendation} disabled={aiLoading}>
                  {t('glossary.ai_retry')}
                </Button>
                <Button
                ...
                >
                  {aiLoading ? t('common.processing') : `${t('glossary.ai_add_selected')} (${aiRecommendations.filter((r) => r.selected).length})`}
                </Button>
              </div>
            </div>
          </>
