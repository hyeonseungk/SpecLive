...
export default function GlossaryAddModal({ projectId, userId, onClose, onGlossaryAdded }: GlossaryAddModalProps) {
  const t = useT()
  ...
  const addGlossary = async () => {
    ...
    try {
      ...
      if (validUrls.length) {
        const rows = validUrls.map((url) => ({ glossary_id: glossary.id, url: url.trim() }))
        const { error: linksErr } = await supabase.from('glossary_links').insert(rows)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        ...
        <div className="flex justify-end gap-2 mt-6">
          <Button variant="outline" onClick={handleClose} disabled={glossarySaving}>
            {t('buttons.cancel')}
          </Button>
          <Button
            onClick={addGlossary}
